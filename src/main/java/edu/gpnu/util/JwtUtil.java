...
public class JwtUtil {
	 ...
	 * @return 是否正确
	 */
	public static boolean verify(String token, String studentId, String secret) {
		try {
			// 根据密码生成JWT效验器
			Algorithm algorithm = Algorithm.HMAC256(secret);
			JWTVerifier verifier = JWT.require(algorithm).withClaim("studentId", studentId).build();
			// 效验TOKEN
	...
	public static String getStudentId(String token) {
		try {
			...
			return jwt.getClaim("studentId").asString();
	 ...
	 * @return 加密的token
	 */
	public static String sign(String studentId, String secret) {
		Date date = new Date(System.currentTimeMillis() + EXPIRE_TIME);
		Algorithm algorithm = Algorithm.HMAC256(secret);
		// 附带username信息
		return JWT.create().withClaim("studentId", studentId).withExpiresAt(date).sign(algorithm);
	...
	public static String getUserNameByToken(HttpServletRequest request) throws RuntimeException {
		...
		String username = getStudentId(accessToken);
	...
	*/
	public static void main(String[] args) {
		String sign = JwtUtil.sign("1", "456");
		System.out.println(sign);
		String token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdHVkZW50SWQiOiIxIiwiZXhwIjoxNTkxMzM0MjAyfQ.nJHgo0V5MLffBf-iEuTbSho9vxvLVcH0GvZYYNnaOTE";
		System.out.println(JwtUtil.getStudentId(token));
	}
}
